...
// -----  Translational correlation functions ----

double ** _tcf;
double ** _tcf_sum;

void tcf_malloc(void);
void tcf_init  (void);
void tcf_mfree (void);
void tcf_reset (int);
...
//extern "C" void vcord_(double *Eulang, double *RCOM, double *Rpt, double *vtable, double *vpot3d, double *radret, double *theret, double *chiret, double *hatx, double *haty, double *hatz, int *ivcord);
...
void rcf_mfree(void)
// memory release after the computation of the rotational correlation function
{
   free_doubleMatrix(_rcf);
   free_doubleMatrix(_rcf_sum);
}

void rcf_reset(int mode) //set everything to 0 before starting a new block
{
#ifdef DEBUG_PIMC
   const char *_proc_= __func__; //  rcf_reset() 
...
void tcf_malloc(void)
// memory allocation for translational correlation functions
{
  _tcf     = doubleMatrix(1,NumbTimes);
  _tcf_sum = doubleMatrix(1,NumbTimes);
}

void tcf_mfree(void)
// memory release after the computation of the translational correlation function
{
   free_doubleMatrix(_tcf);
   free_doubleMatrix(_tcf_sum);
}

void tcf_reset(int mode) //set everything to 0 before starting a new block
{
#ifdef DEBUG_PIMC
   const char *_proc_= __func__; //  tcf_reset() 
...
{
//  cout << "Laura start GetPotEnergy_Densities"<<endl;
...
//----------- [ATOM - MOLECULE] ----------------------
         ...
         {
         //  type 1 is a molecule 
//       cout << "Laura no deberia salir (atom-molecule)"<<endl;
             int sgn   = 1;             // set to -1 to correct the orientaion of dr
...
//----------- [ATOM - NON-LINEAR MOLECULE] ----------------------
        ...
        {
//  cout << "Laura atom-nonlinear molecule"<<endl;
            ...
            {
//  cout << "Laura start if molecule 2"<<endl;
...
//  cout << "Laura ned if molecule 2"<<endl;
            }
            else
            {
//             determine type of atoms for bin_3Ddensity
...
//------------- [ATOM - ATOM] ------------------------------- 
         ...
         {
//  cout << "Laura no deberia salir (atom-atom)"<<endl;
...
//
{
   ...
   {
      int t0 = offset +  it0;
      for (int itc=0;itc<NumbTimes;itc++)  // offsets
      {
          int tc = offset + (it0 + itc) % NumbTimes;
          double p0 = 0.0;
          for (int id=0;id<NDIM;id++)
          p0 += (MCCoords[id][t0]*MCCoords[id][tc]);

         _tcf     [0][itc] += p0;   // block average
         _tcf_sum [0][itc] += p0;   // total average  

      } // END offsets  
   }    // END average over the time origin 
}

void SaveTCF(const char fname [], double acount, int mode)
//  save translational correlation functions
//  
//  mode:  MC_TOTAL - accumulated averages
//  mode:  MC_BLOCK - block averages
//
{ 
  ...
  fid.open(ftcf.c_str(),ios::out); io_setout(fid);
  
  double norm = acount*(double)NumbTimes;
  ...
  for (int it=0;it<=NumbTimes;it++)    // save <n(tau)n(0)>
  {       
     fid << setw(IO_WIDTH) << (double)it*MCTau << BLANK; 
     fid << setw(IO_WIDTH) << tcf_save[0][it % NumbTimes]/norm << BLANK;
...
// dtype == density type
{
//  cout << "Laura start bin3Ddensity"<<endl;
   int bin_r = (int)floor((r-_min_radius)/_delta_radius);
   if ((bin_r<MC_BINSR) && (bin_r>=0))
   {
//  cout << "Laura start if 1"<<endl;
      int bin_t    = (int)floor(theta/_delta_theta);
      if ((bin_t<MC_BINST) && (bin_t>=0))
      {
//  cout << "Laura start if 2"<<endl;
         int bin_c = (int)floor(chi/_delta_chi);
         if((bin_c < MC_BINSC) && (bin_c >= 0))
         {
//  cout << "Laura start if 3"<<endl;
           int ijk = ((bin_r * MC_BINST) + bin_t) * MC_BINSC + bin_c;
//  cout << "Laura start gr3D"<<endl;
           _gr3D[dtype][ijk] += 1.0; // block average
//  cout << "Laura end gr3D and start gr3D_sum"<<endl;
           _gr3D_sum[dtype][ijk] += 1.0; // total average
//  cout << "Laura end gr3D_sum and if 3"<<endl;
         }
//  cout << "Laura end if 2"<<endl;
      }
//  cout << "Laura end if 1"<<endl;
   }
//  cout << "Laura end bin3Ddensity"<<endl;
}
