...
//   cout << "Laura new pass" << passTotal<<endl;

//         toby's printing
//           cout<< "Laura   " <<blockCount<<" "<<passCount<<" "<<time<<endl;
           for (int type=0;type<NumbTypes;type++)
           if (WORM && (type == Worm.type)) 
           {
//  cout << "Laura do MCWormMove"<<endl;
              MCWormMove();
//  cout << "Laura end MCWormMove"<<endl;
              if (!Worm.exists)
              {
/* reactive */
//  cout << "Laura start worm exists"<<endl;
...
#endif
//  cout << "Laura end worm exists"<<endl;
              }
           } 
           else
           {
//  cout << "Laura no deberia salir"<<endl;
           PIMCPass(type,time);
           }

           if (blockCount>NumberOfEQBlocks)        // skip equilibration steps
           {
//  cout << "Laura start averages"<<endl;
// evaluate averages 
              if (passTotal % MCSKIP_AVERG == 0)   // skip correlated configurations
              {
                 if (WORM)
                 {
//  cout << "Laura start if worm"<<endl;
                    if (!Worm.exists)
                    {
//  cout << "Laura start if worm exists"<<endl;
//  cout << "Laura start GetAverage"<<endl;
                       MCGetAverage();
//  cout << "Laura end GetAverage"<<endl;
//                     prtper_(PIndex,&MCAtom[BSTYPE].numb,&blockCount);

                       // print the instantaneous xyz and prl info for the closed path
/* Laura: do not print the xyz and prl files
                       if(PrintXYZprl)
                       {
...
//  cout << "Laura end if worm exists"<<endl;
                    }
                    else  
                    MCWormAverage();  /// TEST ONLY
//  cout << "Laura end if worm"<<endl;
                 }   
                 else
                 {
//  cout << "Laura no deberia salir 2"<<endl;
                    MCGetAverage();
...
//  cout << "Laura ni este tampoco"<<endl;
                 }
              }
...
// DUMP, save global average. if avergCount = 0, then MCGetAverage is never called in this block.  All Saving steps are skipped.
              ...
              {
//  cout << "Laura start SaveSumEnergy"<<endl;
                 sumsCount += 1.0;                 
                 SaveSumEnergy (totalCount,sumsCount);
//  cout << "Laura end SaveSumEnergy"<<endl;
              }
           }  
//             cout << "Laura end averages"<<endl;
...
//  cout << "Laura start GetKinEnergy"<<endl;
   double skin = GetKinEnergy();           // kin energy
//  cout << "Laura endKinEnergy and start GetPotEnergy_Densities"<<endl;
   double spot = GetPotEnergy_Densities(); // pot energy and density distributions
//  cout << "Laura end GetKinEeregy_densisties"<<endl;
   double srot = 0.0;
...
// check whether there're bosons in the system
   ...
   if (BOSONS) 
   {
//  cout << "Laura start if bosons"<<endl;
      GetExchangeLength();
      GetPermutation();
//  cout << "Laura end if bosons"<<endl;
   }
...
// toby temporarily by-pass the density save for the non-linear rotor
   if( IMPURITY && MCAtom[IMTYPE].molecule == 1)
   {
//Laura: do not print the GRA file every block   SaveDensities1D    (fname.c_str(),avergCount);
...
//Laura: do not print the GRC, GRI and GRT files   SaveRho1D          (fname.c_str(),avergCount,MC_BLOCK);

// SaveDensities3D    (fname.c_str(),avergCount,MC_BLOCK); // this step takes lots of space. temporarily turnned off

//Laura: do not write GTC file every block   SaveRhoThetaChi    (fname.c_str(),avergCount,MC_BLOCK);

// IOxyzAng(IOWrite,fname.c_str());
   }

   if (ROTATION) 
//Laura: do not write RCF file every block   SaveRCF            (fname.c_str(),avergCount,MC_BLOCK); 
