...
def makeexecutionfile(src_dir,TypeCal,src1):
	execution_file_dir  = "/home/x3lou/"+src1
	os.chdir(execution_file_dir)
	print(execution_file_dir)
	call(["make", "clean"])
	if (TypeCal == "PIGS"):
...
def compile_rotmat(src1, path_exit_linden):
	path_enter_linden = "/home/x3lou/"+src1+"linear_prop/"
	os.chdir(path_enter_linden)
	call(["make", "clean"])
	call(["make"])
	os.chdir(path_exit_linden)

def compile_cagepot(src1, path_exit_cagepot):
	path_enter_cagepot = "/home/x3lou/"+src1+"tabulated_potential/"
...
def GetAverageEnergy(TypeCal,numbbeads,variable,final_dir_in_work,preskip,postskip):
	...
	if (TypeCal == "PIMC"):
		...
		col_block, col_kin, col_pot, col_kin_pot, col_rot, col_kin_pot_rot = genfromtxt(final_dir_in_work+"/results/output.eng",unpack=True, usecols=[0,1,2,3,4,6], skip_header=preskip, skip_footer=postskip)
		print(len(col_kin_pot))
	
		mean_kin      = np.mean(col_kin)
		mean_rot      = np.mean(col_rot)
		mean_pot      = np.mean(col_pot)
		mean_kin_pot  = np.mean(col_kin_pot)
		mean_tot      = np.mean(col_kin_pot_rot)
		...
		error_kin_pot = np.std(col_kin_pot,ddof=1)/sqrt(len(col_kin_pot))
		error_tot     = np.std(col_kin_pot_rot,ddof=1)/sqrt(len(col_kin_pot_rot))

		output  = '{0:10d}{1:20.5f}{2:20.5f}{3:20.5f}{4:20.5f}{5:20.5f}{6:20.5f}{7:20.5f}{8:20.5f}{9:20.5f}{10:20.5f}{11:20.5f}'.format(numbbeads, variable, mean_kin, mean_rot, mean_pot, mean_kin_pot, mean_tot, error_kin, error_rot, error_pot, error_kin_pot, error_tot)
		output  += "\n"
...
def fmtAverageEnergy(TypeCal,status,variable):
	...
	if status == "analysis":
		output     ="#"
		if (TypeCal == "PIMC"):
			output    += '{0:^15}{1:^20}{2:^20}{3:^20}{4:^20}{5:^20}{6:^20}{7:^20}{8:^20}{9:^20}{10:^20}{11:^20}'.format('Beads', variable, 'Avg. Translational', 'Avg. rotational', 'Avg. Potential', 'Avg. Kin+Pot', 'Avg. Total', 'Error of Kin', 'Error of Rot', 'Error of Pot', 'Error of kin+pot', 'Error of Total')
		if (TypeCal == "PIGS"):
...
def GetAverageEntropy(numbbeads,variable,final_dir_in_work,preskip,postskip,ENT_TYPE):
	...
	if ENT_TYPE == "SWAPTOUNSWAP":
		...
		output  = '{0:10d}{1:20.5f}{2:20.5f}{3:20.5f}{4:20.5f}{5:20.5f}{6:20.5f}{7:20.5f}{8:20.5f}{9:20.5f}{10:20.5f}{11:20.5f}'.format(numbbeads, variable, mean_nm, mean_dm, purity, mean_EN, error_nm, error_dm, error_Tr, error_EN)
		output  += "\n"
...
def rotmat(TypeCal,molecule,temperature,numbbeads,src1):
	'''
	...
	command_linden_run = "/home/x3lou/"+src1+"/linear_prop/linden.x "+str(temperature)+" "+str(numbbeads1)+" "+str(GetBconst(molecule))+" 15000 -1"
...
def cagepot(src1):
	...
	'''
	command_cagepot_run = "/home/x3lou/"+src1+"tabulated_potential/hfc60.x 100 360"
...
def Submission(status, RUNDIR, dir_run_job, folder_run, src_dir, execution_file, Rpt, numbbeads, i, step, step_trans, level, temperature, numbblocks, numbpass, molecule_rot, numbmolecules, dipolemoment, status_rhomat, TypeCal, dir_output, dir_run_input_pimc, RUNIN, particleA, NameOfPartition, status_cagepot, iStep,src1, POTREAD,dir1):
	...
	if status_rhomat == "Yes":
		rotmat(TypeCal,molecule_rot,temperature,numbbeads,src1)
	...
	if (POTREAD):
		for ipot in range(len(POTREAD)):
			call(["cp", POTREAD[ipot], dir_run_input_pimc])
	...
	if RUNDIR == "scratch":
		if RUNIN == "CPU":
			...
			fwrite.write(jobstring_sbatch(RUNDIR, argument2,i,numbmolecules, folder_run_path, molecule_rot, temperature, numbbeads, final_dir_in_work, dir_run_input_pimc,POTREAD,dir1))
	else: 
		fwrite.write(jobstring_sbatch(RUNDIR, argument2, i, numbmolecules, folder_run_path, molecule_rot, temperature, numbbeads, final_dir_in_work, dir_run_input_pimc,POTREAD,dir1))
...
def jobstring_sbatch(RUNDIR, file_name, value, thread, folder_run_path, molecule, temperature, numbbeads, final_dir_in_work, dir_run_input_pimc,POTREAD,dir1):
	'''
	...
	if (RUNDIR == "scratch"):
		CommandForMove = "mv "+str(folder_run_path)+" /work/x3lou/"+dir1
	if (RUNDIR == "work"):
		CommandForMove = " "
	if (POTREAD):
		job_string2 = ""
		for ipot in range(len(POTREAD)):
			filePot = dir_run_input_pimc+"/"+POTREAD[ipot]
			job_string2 +="""cp %s %s;""" % (filePot, folder_run_path)
		

	job_string1     = """#!/bin/bash
#SBATCH --job-name=%s
#SBATCH --output=%s.out
#SBATCH --time=%s
...
mv %s %s
mv %s %s
""" % (job_name, logpath, walltime, omp_thread, omp_thread, folder_run_path, output_dir, cagepot_file, folder_run_path, input_file, folder_run_path, file_rotdens, folder_run_path)
...
./pimc 
%s
""" % (folder_run_path, qmcinp, exe_file, folder_run_path, CommandForMove)
	job_string=job_string1+job_string2+job_string3
	return job_string
...
class GetFileNameAnalysis:
	def __init__(self, TypeCal1, molecule_rot1, TransMove1, RotMove1, variableName1, Rpt1, dipolemoment1, parameterName1, parameter1, numbblocks1, numbpass1, numbmolecules1, molecule1, ENT_TYPE1, preskip1, postskip1, extra1, src_dir1, particleA1):
		...
		if ((self.TypeCal == "PIGS") or (self.TypeCal == "PIMC")):
			...
			if (self.molecule_rot == "H2" or "CO2"):
