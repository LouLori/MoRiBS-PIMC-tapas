...
#endif
//         save translational correlation function
           SaveTCF(MCFileName.c_str(),totalCount,MC_TOTAL);
...
//Laura: do not write TCF file every block   SaveTCF (fname.c_str(),avergCount,MC_BLOCK);
...
void SaveInstantAngularDOF(long int numb)
{
	...
	double cntrdR    = 0.0;
	double scosTheta = 0.0;
    for (int it = 0; it < NumbTimes; it++) // Rotational Time slices, P
    {
        int offset0 = MCAtom[0].offset;
        int offset1 = MCAtom[1].offset;
        ...
        {
            dr   = (MCCoords[id][t1] - MCCoords[id][t0]);
		    dr2 += dr*dr;
        }
		double r = sqrt(dr2);
		double sumcost = 0.0;
        for (int id=0;id<NDIM;id++)
        {
			sumcost += (MCCoords[id][t1] - MCCoords[id][t0])*MCCosine[id][t1]/r;
        }
        _fangins << setw(IO_WIDTH) << sumcost << BLANK;
        _fangins << setw(IO_WIDTH) << r << BLANK;
	    _fangins << endl;
		scosTheta += sumcost;
		cntrdR += r;
	}
   	...
   	_fanginsc << setw(IO_WIDTH) << scosTheta/NumbTimes << BLANK;
   	_fanginsc << setw(IO_WIDTH) << cntrdR/NumbTimes << BLANK;
    _fanginsc << endl;
}
